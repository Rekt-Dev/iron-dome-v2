...
const S = {
  root:       { background:"#080808", minHeight:"100dvh", color:"#e2e8f0", fontFamily:"'DM Mono','Courier New',monospace", padding:"16px 14px 60px", maxWidth:520, margin:"0 auto", boxSizing:"border-box" },
  header:     { display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 },
  title:      { fontSize:22, fontWeight:700, letterSpacing:"0.08em", color:"#fff" },
  date:       { fontSize:12, color:"#94a3b8", marginTop:3 },

  statsStrip: { display:"flex", background:"#111", border:"1px solid #1e1e1e", borderRadius:10, padding:"10px 0", marginBottom:12 },
  stat:       { flex:1, display:"flex", flexDirection:"column", alignItems:"center" },
  statVal:    { fontSize:20, fontWeight:700, color:"#f1f5f9" },
  statLbl:    { fontSize:10, color:"#475569", textTransform:"uppercase", letterSpacing:"0.1em", marginTop:2 },
  divider:    { width:1, background:"#1e1e1e" },

  restBar:    { background:"#0f0f0f", border:"1px solid", borderRadius:10, padding:"12px 14px", marginBottom:12 },
  restTrack:  { height:4, background:"#1a1a1a", borderRadius:2, overflow:"hidden" },

  backupBtn:  { background:"#141414", border:"1px solid #252525", color:"#94a3b8", padding:"8px 14px", borderRadius:8, fontSize:13, cursor:"pointer", fontFamily:"inherit" },
  overlay:    { position:"fixed", inset:0, zIndex:99 },
  backupMenu: { position:"absolute", right:0, top:"110%", background:"#121212", border:"1px solid #252525", borderRadius:10, padding:10, zIndex:100, minWidth:175, boxShadow:"0 16px 48px rgba(0,0,0,0.9)" },
  menuSection:{ fontSize:10, color:"#475569", letterSpacing:"0.12em", marginBottom:6, textTransform:"uppercase" },
  ...
  bStatus:    c => ({ fontSize:11, color:c }),

  card:       { background:"#0f0f0f", border:"1px solid #1a1a1a", borderRadius:14, padding:"16px 14px", marginBottom:14 },
  chips:      { display:"flex", flexWrap:"wrap", gap:6, marginBottom:14 },
  chip:       { background:"#161616", border:"1px solid #1e1e1e", color:"#64748b", borderRadius:20, padding:"6px 12px", fontSize:12, cursor:"pointer", fontFamily:"inherit" },
  chipActive: { background:"rgba(96,165,250,0.12)", border:"1px solid rgba(96,165,250,0.4)", color:"#93c5fd" },

  grid:       { display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 },
  fieldLabel: { display:"flex", flexDirection:"column", gap:5 },
  fieldName:  { fontSize:10, color:"#64748b", letterSpacing:"0.1em", textTransform:"uppercase" },
  inputRow:   { display:"flex", alignItems:"center", gap:4 },
  input:      { flex:1, background:"#1a1a1a", border:"1px solid #252525", color:"#f1f5f9", padding:"11px 6px", borderRadius:8, fontSize:18, fontFamily:"inherit", boxSizing:"border-box", textAlign:"center", width:"100%" },
  step:       { background:"#1e1e1e", border:"1px solid #252525", color:"#94a3b8", borderRadius:6, padding:"10px 10px", fontSize:16, cursor:"pointer", fontFamily:"inherit", lineHeight:1, flexShrink:0 },

  modeBtn:    { width:"100%", background:"transparent", border:"2px solid", padding:"13px", borderRadius:10, fontSize:13, fontFamily:"inherit", cursor:"pointer", letterSpacing:"0.12em", fontWeight:700, marginBottom:10 },
  addBtn:     { width:"100%", background:"#4ade80", border:"none", color:"#000", padding:"15px", borderRadius:10, fontSize:15, fontWeight:700, fontFamily:"inherit", cursor:"pointer", letterSpacing:"0.1em" },

  log:        { background:"#0a0a0a", border:"1px solid #161616", borderRadius:14, padding:"14px 12px" },
  logTitle:   { fontSize:10, color:"#334155", letterSpacing:"0.18em", marginBottom:16, textTransform:"uppercase" },
  empty:      { color:"#334155", fontSize:13, textAlign:"center", padding:"28px 0" },

  exBlock:    { marginBottom:20 },
  exHead:     { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 },
  exName:     { fontSize:15, fontWeight:700, color:"#f1f5f9", letterSpacing:"0.04em" },
  volBadge:   { fontSize:11, color:"#4ade80", background:"rgba(74,222,128,0.1)", padding:"3px 10px", borderRadius:99, fontWeight:600 },

  setRow:     { display:"flex", alignItems:"center", gap:8, padding:"9px 0", borderBottom:"1px solid #111" },
  setNum:     { fontSize:11, color:"#475569", width:16, textAlign:"right", flexShrink:0 },
  setMain:    { fontSize:16, color:"#e2e8f0", fontWeight:600, minWidth:90 },
  tag:        { fontSize:10, color:"#94a3b8", background:"#161616", padding:"2px 7px", borderRadius:6 },
  delBtn:     { background:"transparent", border:"none", color:"#475569", cursor:"pointer", fontSize:13, padding:"4px 6px", marginLeft:"auto", flexShrink:0 },
};
